...
void main() {
    // 计算线段生长的进度（0.0-1.0）
    float growthProgress = clamp(currentTime / growthDuration, 0.0, 1.0);
    ...
    float flowAnimationTime = max(0.0, currentTime - growthDuration);
    ...
    vec4 gradientColor = vec4(vLineColor, alpha);
